...
interface TaskSectionProps {
  tasks: Task[];
  users: User[];
  // FIX: Update onAddTask prop type to not expect id, createdBy, or createdAt, as they are generated by the parent.
...
const TaskSection: React.FC<TaskSectionProps> = ({ tasks, users, onAddTask, onRemoveTask, onUpdateTaskStatus }) => {
  ...
  const [assignedTo, setAssignedTo] = useState<string>(users[0]?.id.toString() ?? '');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (description && assignedTo && dueDate) {
      onAddTask({ description, assignedTo: parseInt(assignedTo), dueDate, status: 'To Do' });
      setDescription('');
      setDueDate('');
    }
  };
